...
def execute_sql_via_rest(url, key, sql):
    ...
    # Supabase has a REST endpoint for executing SQL
    # We'll use the rpc endpoint to call a custom function
    ...
    try:
        # Try to execute the function creation first
        ...
        print(f"⚠️  Could not create exec_sql function: {e}")

    # Now try to execute the main SQL
    try:
        response = requests.post(
            f"{url}/rest/v1/rpc/exec_sql",
            headers=headers,
