...
export function DimensionCard({ dimension, index }: DimensionCardProps) {
    ...
    return (
        <motion.div
            ...
            className={`relative group rounded-3xl p-1 transition-all duration-300 ${isHovered ? "z-20 scale-105" : "z-10"
                }`}
        >
            {/* Animated Border Gradient */}
            <div
                className={`absolute inset-0 rounded-3xl opacity-20 group-hover:opacity-100 transition-opacity duration-500 blur-xl ${dimension.type === "soft"
            ...
            {/* Card Content */}
            <div className="relative h-full bg-[#0A0F1A]/90 backdrop-blur-xl border border-white/10 rounded-[1.4rem] overflow-hidden flex flex-col">
                {/* Background Mesh Gradient */}
                <div
                    className={`absolute top-0 right-0 w-64 h-64 rounded-full blur-[80px] opacity-20 transition-all duration-500 group-hover:opacity-40 ${dimension.type === "soft"
                        ? "bg-brand-blue group-hover:bg-brand-accent"
                        : "bg-its-gold group-hover:bg-amber-400"
                ...
                <div className="p-8 flex flex-col h-full relative z-10">
                    ...
                    <div className="flex justify-between items-start mb-4">
                        <div
                            className={`w-16 h-16 rounded-2xl flex items-center justify-center transition-all duration-300 group-hover:scale-110 group-hover:rotate-3 shadow-lg ${dimension.type === "soft"
