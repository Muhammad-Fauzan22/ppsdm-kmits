      ...
      <div className="max-w-3xl mx-auto">
            ...
            <div className="grid grid-cols-5 gap-3">
              {[1, 2, 3, 4, 5].map((value) => (
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  className={`p-4 rounded-xl border-2 transition-all ${localSelected === value
        ...
        <div className="flex items-center justify-between">
          ...
          >
            ...
            {questionNumber >= totalQuestions - 1 ? <Check className="w-5 h-5" /> : <ArrowRight className="w-5 h-5" />}
          </button>
        </div>
        ...
        <div className="flex justify-center gap-2 mt-6">
          {Array.from({ length: totalQuestions }).map((_, i) => (
            <div
              key={i}
              className={`w-2 h-2 rounded-full transition-all ${i <= questionNumber
                ? 'bg-brand-accent'
                : 'bg-white/20'
                }`}
            />
