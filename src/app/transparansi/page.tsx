...
import { RefreshCw, Eye, DollarSign, Calendar, Users, FileText } from "lucide-react";
...
export default function TransparansiPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const activitiesRes = await fetch("/api/google-sheets/data?sheet=activities");
      const activitiesData = await activitiesRes.json();
      
      // Fetch finances data
      const financesRes = await fetch("/api/google-sheets/data?sheet=finances");
    ...
    return (
      <div className="container mx-auto p-4 flex justify-center items-center h-64">
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-muted-foreground">Memuat data transparansi...</p>
        </div>
      </div>
    );
  }
