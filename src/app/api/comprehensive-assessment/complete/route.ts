...
export async function POST(request: NextRequest) {
    try {
        ...
        const summary = {
            ...
            critical_areas: gaps?.filter((g) => g.priority_level === "critical").length || 0,
            high_priority: gaps?.filter((g) => g.priority_level === "high").length || 0,
            moderate_areas: gaps?.filter((g) => g.priority_level === "moderate").length || 0,
            minimal_gaps: gaps?.filter((g) => g.priority_level === "minimal").length || 0,
        };
...
export async function GET(request: NextRequest) {
    try {
        ...
        // Group by priority
        const byPriority = {
            critical: gaps?.filter((g) => g.priority_level === "critical") || [],
            high: gaps?.filter((g) => g.priority_level === "high") || [],
            moderate: gaps?.filter((g) => g.priority_level === "moderate") || [],
            minimal: gaps?.filter((g) => g.priority_level === "minimal") || [],
        };
        ...
        return NextResponse.json({
            ...
            summary: {
                ...
                avgGap: gaps?.reduce((sum, g) => sum + g.gap_score, 0) / (gaps?.length || 1),
            },
        });
    } catch (error) {
