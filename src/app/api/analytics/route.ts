...
export async function GET() {
    try {
        ...
        const inProgress = activities?.filter(a => a.status === "in-progress").length || 0;
        const totalPoints = activities?.filter(a => a.status === "completed").reduce((acc, a) => acc + (a.points || 0), 0) || 0;
        ...
        const growthHistory = months.map((month, i) => {
            const avgScore = scores?.length ? Math.round(scores.reduce((acc, s) => acc + s.score, 0) / scores.length) : 0;
            return {
                month,
