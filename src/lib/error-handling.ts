  ...
  if (error instanceof PPSDMError) {
    switch (error.severity) {
      case ErrorSeverity.CRITICAL:
        logger.error('CRITICAL_ERROR', error, errorInfo);
        break;
      case ErrorSeverity.HIGH:
        logger.error('HIGH_PRIORITY_ERROR', error, errorInfo);
        break;
    ...
    logger.error('UNHANDLED_ERROR', error, errorInfo);
  }

  // Log to API logger if context is available
  if (context?.method && context?.url) {
    logger.info(`API Error: ${context.method} ${context.url}`, {
...
export async function withRetry<T>(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      // Don't retry if it's not a retryable error or it's last attempt
      if (
      ...
      logger.warn(`RETRY_ATTEMPT ${attempt}/${maxAttempts}`, {
        delay: currentDelay,
...
export function setupGracefulShutdown(callback: () => Promise<void>) {
  const shutdown = async (signal: string) => {
    ...
    try {
      ...
      logger.error('GRACEFUL_SHUTDOWN_ERROR', error as Error);
      process.exit(1);
    }
  };
